...
VertexInterface::VertexInterface(int idx, int x, int y, std::string pic_name, int pic_idx)
{
    ...
    std::cout<<"On est al frere"<<std::endl;
}
...
GraphInterface::GraphInterface(int x, int y, int w, int h)
{
    ...
    ///boutton d'add de sommet
    m_top_box.add_child(ajout_sommet);
    ajout_sommet.set_pic_name("boutton_rouge.jpg");
    ajout_sommet.set_posx(0);
    ajout_sommet.set_posy(0);

    ///boutton d'add d'arc
    m_top_box.add_child(ajout_arc);
    ajout_arc.set_pic_name("boutton_orange.jpg");
    ajout_arc.set_posx(0);
    ajout_arc.set_posy(100);

    ///boutton de sauvegarde
    m_top_box.add_child(sauvegarde);
    sauvegarde.set_pic_name("boutton_vert.jpg");
    sauvegarde.set_posx(0);
    sauvegarde.set_posy(200);

    ///boutton d'add d'arc
    m_top_box.add_child(del_arc);
    del_arc.set_pic_name("boutton_violet.jpg");
    del_arc.set_posx(0);
    del_arc.set_posy(300);

    ///boutton d'add d'arc
    m_top_box.add_child(del_som);
    del_som.set_pic_name("boutton_bleu.jpg");
    del_som.set_posx(0);
    del_som.set_posy(400);
}
...
void Graph::update(Graph g, std::string NomFichier)
{
    if (!m_interface)
        return;
    ...
    for (auto &elt : m_edges)
        {elt.second.post_update();}
    std::cout<<"Bienvenue update"<<std::endl;
        ///bouton sa mere
        ///Ajout de sommet
        if(m_interface->ajout_sommet.get_value())
        {
            ajouterSommet(NomFichier);
        }
        ///Ajout d'arc
        if(m_interface->ajout_arc.get_value())
        {
            ajouterArc(NomFichier);
        }
        ///Sauvegarder
        if(m_interface->sauvegarde.get_value())
        {
            sauvegarde(NomFichier);
        }
        ///Suppression d'arc
        if(m_interface->del_arc.get_value())
        {
            test_remove_edge();
        }
        ///Suppression de sommet
        if(m_interface->del_som.get_value())
        {
...
{
    m_interface->sauvegarde.interact_focus();
    std::string number;
    int pos_sommetx;
    int pos_sommety;
...
{
    std::ifstream fichier(NomFichier+".txt",std::ios::in);
    if(fichier)
    {
        m_interface = std::make_shared<GraphInterface>(50, 0, 750, 600);
        int nb_sommet;
       ...
       for(int i=0;i<nb_arete;i++)
       {
        ...
        ///boutton ajouter sommet (nécessité de decla dans graph.h)
    }
    fichier.close();
}
...
void Graph::ajouterSommet(std::string NomFichier)
{
    m_interface->ajout_sommet.interact_focus();
    ...
    add_interfaced_vertex(nb_sommet, z, 0, 0,nom_sommet);
}

void Graph::ajouterArc(std::string NomFichier)
{
    /*
    int nb_sommet;
    int nb_arc;
    */
    m_interface->ajout_arc.interact_focus();
    int sommet1,sommet2;
    std::cout << "rentrez les numéros des sommets dans cet ordre : X mange Y" << std::endl;
    std::cin >> sommet1;
    std::cin >> sommet2;
...
void Graph::test_remove_edge()
{
    m_interface->del_arc.interact_focus();
    int eidx;
...
/*void fichier(std::string NomFichier, Graph g, int sommet_indic, int arc_indic)
{
    int a, num;
    g.chargement(NomFichier);
    while ( !key[KEY_ESC] )
        {
            ...
            if( key[KEY_RIGHT] )
                {
                    ...
                    if(a==1)
                        {
                            g.ajouterSommet(NomFichier, sommet_indic);
                        }
                    else if(a==2)
                        {
                            g.ajouterArc(NomFichier, arc_indic);
                        }
                    if (a==4)
                        {
                            std::cout << " Quel arc voulez vous supprimer ?" << std::endl;
